...
def hourglass(window, n, point, radius, color):
    ...
    for k in range(n):
        for j in range(k + 1):
            ...
            lineAbove = rg.Line(rg.Point(circleAbove.center.x - radius, circleAbove.center.y),
                                rg.Point(circleAbove.center.x + radius, circleAbove.center.y))
            lineBelow = rg.Line(rg.Point(circleBelow.center.x - radius, circleBelow.center.y),
                                rg.Point(circleBelow.center.x + radius, circleBelow.center.y))
